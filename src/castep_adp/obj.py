...
class MD:
  ...
  def apply_units(self):
    self.positions *= constants.ureg.atomic_unit_of_length
    self.velocities *= (constants.ureg.atomic_unit_of_length/constants.ureg.atomic_unit_of_time)
...
class Cell:
  ...
  def get_positions_abs(self):
    ...
    for key in self.positions_frac.keys():
      pos = np.zeros(3,dtype=float)*constants.ureg.angstrom
      for i in range(3):
  ...
  def construct_cart(self):
    start = 1
    unit = constants.ureg.angstrom
    if len(self.lattice) == 6:
      if self.lattice[1] in constants.units_dict.keys():
        unit = constants.units_dict[self.lattice[1]][0]
      else:
      ...
      start += 1
    lattice = np.zeros((3,3),dtype=float)*constants.ureg.angstrom
  ...
  def construct_abs(self):
    ...
    for line in self.positions[1:-1]:
      line = line.split()
      if len(line)==1:
        if line[0] in constants.units_dict.keys():
          unit = constants.units_dict[line[0]][0]
        else:
          raise err.UnitError(kind="unknown unit",loc="cell file",unit=self.lattice[1])
        if unit.dimensionality != "[length]":
          raise err.UnitError(kind="dimensionality",unit=unit,dim="[length]")
